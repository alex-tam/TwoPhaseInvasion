...
@with_kw struct Params
    ...
    αv::Float64 = 0.5 # [-] Maximum initial density (v)
...
function ic(par::Params, x, y)
    ...
    for i in eachindex(x)
        for j in eachindex(y)
            # Ellipse
            if (1/16*(x[i]-par.Lx/2)^2 + 1/9*(y[j]-par.Ly/2)^2)< par.β
                U[i,j] = par.αu # Constant density (inside Ω1)
            elseif (1/16*(x[i]-par.Lx/2)^2 + 1/9*(y[j]-par.Ly/2)^2) == par.β
                U[i,j] = par.uf # Constant density (on interface)
            else
                U[i,j] = par.αv # Initial density (outside Ω1)
            end
            ϕ[i,j] = (1/16*(x[i]-par.Lx/2)^2 + 1/9*(y[j]-par.Ly/2)^2)- par.β # Initial signed-distance
