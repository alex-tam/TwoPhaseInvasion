...
function draw_slices(x, y, nx, ny, Lx, Ly, plot_times)
    ...
    for i in plot_times
        ...
        vx = v[:, ny]
        # Plot data
        Plots.plot!(x, ux, xlabel = L"$x$", ylabel = L"$u(x,10,t)$", linecolor=:black, linewidth = 1, aspect_ratio = Lx, grid = false, margin=3mm, legend=false, xlims=(0, Lx), ylims=(0,1.2))
    ...
    for i in plot_times
        ...
        vy = v[nx, :]
        # Plot data
        Plots.plot!(y, uy, xlabel = L"$y$", ylabel = L"$u(10,y,t)", linecolor=:black, linewidth = 1, aspect_ratio = Ly, grid = false, margin=3mm, legend=false, xlims=(0, Ly), ylims=(0,1.2))
