...
function fkpp_rhs!(du, u, p, t, D, dΩ, ϕ, uf, y, par, dx, dy)
    ...
    for i in eachindex(D) # Loop over grid points
        if (D[i].yInd != 1) && (D[i].yInd != par.Ny) && (D[i].Ω == true) && (D[i].dΩ == false) # Interior grid points inside Ω1, away from dΩ
            ...
            du[i] = par.Du*(uxx + uyy) + par.λu*u_mid*(1-u_mid)
        elseif ((D[i].yInd == 1) || (D[i].yInd == par.Ny)) && (D[i].Ω == true) && (D[i].dΩ == false) # Boundary grid points inside Ω1, away from dΩ
            # Obtain density at stencil points
            u_mid = U[D[i].xInd, D[i].yInd]
            if ϕ[D[i].xInd-1, D[i].yInd] > 0
                u_left = get_interface_density(D[i].xInd-1, D[i].yInd, D[i].xInd, D[i].yInd, dΩ, uf)
            ...
            uyy = 2.0*( u_bottom/(D[i].θym*(D[i].θym+D[i].θyp)) - u_mid/(D[i].θym*D[i].θyp) + u_top/(D[i].θyp*(D[i].θym+D[i].θyp)) )/(dy^2)
            du[i] = par.Du*(uxx + uyy) + par.λu*u_mid*(1-u_mid)
        elseif (D[i].yInd != 1) && (D[i].yInd != par.Ny) && (D[i].Ω == false) && (D[i].dΩ == false) # Interior grid points outside Ω1, away from dΩ
            # Obtain density at stencil points
            u_mid = U[D[i].xInd, D[i].yInd]
            if ϕ[D[i].xInd-1, D[i].yInd] < 0
                u_left = get_interface_density(D[i].xInd-1, D[i].yInd, D[i].xInd, D[i].yInd, dΩ, uf)
            ...
            uxx = 2.0*( u_left/(D[i].θxm*(D[i].θxm+D[i].θxp)) - u_mid/(D[i].θxm*D[i].θxp) + u_right/(D[i].θxp*(D[i].θxm+D[i].θxp)) )/(dx^2)
            uyy = 2.0*( u_bottom/(D[i].θym*(D[i].θym+D[i].θyp)) - u_mid/(D[i].θym*D[i].θyp) + u_top/(D[i].θyp*(D[i].θym+D[i].θyp)) )/(dy^2)
            du[i] = par.Dv*(uxx + uyy) + par.λv*u_mid*(1-u_mid)
        # Grid points outside Ω or close to dΩ
        elseif ((D[i].yInd == 1) || (D[i].yInd == par.Ny)) && (D[i].Ω == false) && (D[i].dΩ == false) # Boundary grid points outside Ω1, away from dΩ
            # Obtain density at stencil points
            u_mid = U[D[i].xInd, D[i].yInd]
            if ϕ[D[i].xInd-1, D[i].yInd] < 0
                u_left = get_interface_density(D[i].xInd-1, D[i].yInd, D[i].xInd, D[i].yInd, dΩ, uf)
